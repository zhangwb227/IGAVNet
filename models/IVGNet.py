...
class GAMBlock(nn.Module):
    ...
    def __init__(self, in_channel, device, block_size_w=35,block_size_h=35, ratio=4):
    ...
    def forward(self, x):
        ...
        for i in range(x2.shape[0]):
            ...
            # 将图像分割成块
            img_xblock = xi // self.block_size_w
            img_yblock = yi // self.block_size_h
            for xk in range(img_xblock):  # 控制当前图像‘长’可以分为多少个块
                for yk in range(img_yblock):  # 控制当前图像‘宽’可以分为多少个块
                    # 计算图中的边
                    a = img_label[xk * self.block_size_w:(xk + 1) * self.block_size_w, yk * self.block_size_h:(yk + 1) * self.block_size_h]
                    a = a.reshape(1,-1)
                    a = a.T @ a
                    index = torch.argwhere(a > 0.25)
                    if index.numel()>=2:
                        # 创建图
                        g = dgl.graph((index[:,0], index[:,1]), num_nodes=self.block_size_w * self.block_size_h)
                        g = dgl.add_self_loop(g)
                    else:
                        # 创建图
                        g = dgl.graph(([], []), num_nodes=self.block_size_w * self.block_size_h)
                        g = dgl.add_self_loop(g)
                    # 计算节点特征
                    f = x2[i][:, xk * self.block_size_w:(xk + 1) * self.block_size_w, yk * self.block_size_h:(yk + 1) * self.block_size_h]
                    f = f.reshape(ci, self.block_size_w * self.block_size_h)
                    # 赋予节点特征
...
class IVGNet(nn.Module):
    def __init__(self, in_size, out_size, n_classes, middle_size, device,block_size_w=35,block_size_h=35):
        super(IVGNet, self).__init__()
        self.in_size = in_size 
        self.out_size = out_size
        ...
        self.fgat3 = GAMBlock(self.out_size,device,block_size_w,block_size_h)
        self.u_d_maxpool3 = nn.MaxPool2d(kernel_size=2)
        
        self.fgat4 = GAMBlock(self.out_size,device,block_size_w,block_size_h)
        self.u_d_maxpool4 = nn.MaxPool2d(kernel_size=2)
        
        self.fgat5 = GAMBlock(self.out_size,device,block_size_w,block_size_h)
